...
import { askConcernNode, normalizeConcernNode, atomicConcernNode, resolveAtomicConcernNode } from "./workFlowNodes.js";   

export function buildDiscoveryGraph(deps){
    const discoveryGraph = new StateGraph({ channels: stateChannels  });
    ...
    discoveryGraph.addNode("atomizeConcern", (state) => atomicConcernNode(state));
    discoveryGraph.addNode("resolveAtomicConcerns", (state) => resolveAtomicConcernNode(state, deps));
